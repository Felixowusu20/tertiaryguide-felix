...
export function ProgrammesSection({ slug, onError }: Props) {
  ...
  const saveProgramme = async (e: React.FormEvent) => {
    ...
    try {
      const res = await adminFetch(
        editingId
          ? `/api/school-portal/${slug}/programmes/${editingId}`
          : `/api/school-portal/${slug}/programmes`,
        {
      ...
      onError(err instanceof Error ? err.message : "Could not save programme");
    } finally {
      setBusy(false);
    }
  };
  ...
  const startEdit = (p: Programme) => {
    ...
    setIntakeOptions((p.intakeOptions || []).join(", "));
    setCutoff(p.cutoff || "");
    ...
    <section className="space-y-4">
      <div className="rounded-3xl border border-[#DBEAFE] bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-[#6B7280]">
          Programmes, cutoffs and prerequisites appear on the student form and in programme comparison.
        </p>
        <form onSubmit={saveProgramme} className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <label className="space-y-1 text-sm sm:col-span-2">
            <span className="font-medium">Prerequisite (needed for comparison)</span>
            <textarea
              value={preRequisite}
              onChange={(e) => setPreRequisite(e.target.value)}
              rows={2}
              placeholder="e.g. WASSCE credit passes in Core Maths, English and Integrated Science"
              className="w-full rounded-xl border border-[#E5E7EB] px-3 py-2 outline-none focus:border-[#007AFF]"
            />
          </label>
          <label className="space-y-1 text-sm">
            <span className="font-medium">Duration (years, optional)</span>
            <input
              type="number"
              min={1}
              step={1}
          ...
          <div className="flex flex-wrap gap-2 sm:col-span-2">
            <button
              type="submit"
              disabled={busy}
              className="inline-flex items-center justify-center gap-2 rounded-full bg-[#007AFF] px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-60"
            >
              {busy ? (
              ...
              {editingId ? "Save programme" : "Add programme"}
            </button>
            {editingId && (
              <button
                type="button"
                onClick={resetForm}
                className="inline-flex items-center justify-center gap-1.5 rounded-full border border-[#E5E7EB] px-4 py-2.5 text-sm font-medium text-[#374151]"
              >
                <X className="h-4 w-4" />
                Cancel
              </button>
              ...
              <tbody>
                {programmes.map((p) => (
                  <tr key={p.id} className="border-t border-[#F3F4F6]">
                    ...
                    <td className="max-w-[16rem] px-4 py-3 text-xs text-[#4B5563]">
                      {p.preRequisite || "—"}
                    </td>
                    <td className="px-4 py-3">
                      <span
